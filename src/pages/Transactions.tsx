        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[500px] max-h-[95vh] overflow-y-auto p-0 border-none bg-transparent shadow-none">
                <div className="bg-white dark:bg-neutral-900 rounded-xl overflow-hidden shadow-2xl border">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                      ...
                      <div className="space-y-4 bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-lg border border-neutral-100 dark:border-neutral-800">
                        <div className="flex justify-center">
                          <div className="grid grid-cols-2 w-full max-w-[240px] p-1 bg-neutral-200 dark:bg-neutral-800 rounded-lg">
                            <button
                              type="button"
                              onClick={() => setType('despesa')}
                              className={`py-1.5 px-4 rounded-md text-sm font-medium transition-all ${type === 'despesa' ? 'bg-white dark:bg-neutral-700 shadow-sm text-red-600' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                              Despesa
                            </button>
                            <button
                              ...
                              className={`py-1.5 px-4 rounded-md text-sm font-medium transition-all ${type === 'receita' ? 'bg-white dark:bg-neutral-700 shadow-sm text-emerald-600' : 'text-muted-foreground hover:text-foreground'}`}
                            >
                              Receita
                            </button>
                          </div>
                        ...
                        <div className="space-y-3">
                          <div className="space-y-1">
                            ...
                            <div className="relative">
                              ...
                              <Input 
                                ...
                                placeholder="0,00" 
                                className="pl-12 h-14 text-2xl font-bold rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900"
                          ...
                          {recurrenceType === 'parcelada' && (
                            <div className="space-y-1">
                              <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Valor Total</Label>
                              <div className="relative">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xl font-bold text-muted-foreground">R$</span>
                                <Input 
                                  type="number" 
                                  ...
                                  placeholder="0,00" 
                                  className="pl-12 h-11 text-xl font-bold rounded-lg border-neutral-200 dark:border-neutral-700 bg-neutral-100 dark:bg-neutral-800"
                                />
                              </div>
                            </div>
                          )}
                          <div className="space-y-1">
                            <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Descrição</Label>
                            <Input 
                              value={description} 
                              onChange={e => setDescription(e.target.value)} 
                              placeholder="Ex: Supermercado, Aluguel, Salário..." 
                              className="h-11 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900"
                            />
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Método de Pagamento</Label>
                          <Select value={paymentMethod} onValueChange={(v: 'pix' | 'credito' | 'debito' | 'dinheiro') => setPaymentMethod(v)}>
                            <SelectTrigger className="h-11 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900">
                        ...
                        <div className="space-y-2">
                          <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Tipo de Lançamento</Label>
                      ...
                      {paymentMethod === 'credito' && (
                        <div className="space-y-2">
                          <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Qual Cartão?</Label>
                          <Select value={creditCardId} onValueChange={setCreditCardId}>
                            <SelectTrigger className={`h-11 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 ${!creditCardId ? 'border-red-500' : ''}`}>
                          ...
                          {!creditCards.length && (
                            <p className="text-xs text-red-500">Cadastre um cartão nas configurações antes de adicionar despesas de crédito.</p>
                          )}
                        </div>
                      )}
                      ...
                      <div className="space-y-4 bg-neutral-50 dark:bg-neutral-800/50 p-4 rounded-lg border border-neutral-100 dark:border-neutral-800">
                        <div className="space-y-2">
                          ...
                          <Input type="date" value={date} onChange={e => setDate(e.target.value)} className="h-11 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900" />
                        </div>

                        {(recurrenceType === 'assinatura' || recurrenceType === 'fixa') && (
                          <div className="space-y-2">
                            <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Dia de Cobrança Mensal</Label>
                            <div className="flex items-center gap-4">
                              <Input 
                                ...
                                className="h-11 text-center font-bold text-lg w-20 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900"
                              />
                              <span className="text-sm text-muted-foreground">Todo dia {billingDay} de cada mês</span>
                            </div>
                          </div>
                        )}
                        ...
                        {recurrenceType === 'parcelada' && (
                          <div className="space-y-2">
                            <Label className="text-[10px] uppercase font-bold tracking-widest text-muted-foreground">Número de Parcelas</Label>
                            <div className="flex items-center gap-4">
                              <Input 
                                type="number" 
                                min="2" 
                                ...
                                className="h-11 text-center font-bold text-lg w-20 rounded-lg border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900"
                              />
                              <span className="text-sm text-muted-foreground">Vezes (ex: {totalInstallments}x de {formatCurrency(parseFloat(amount) || 0)})</span>
                            </div>
                          </div>
                        )}
                            ...
                            <SelectValue placeholder="Selecione uma categoria">
                              {categoryId ? (
                                <div className="flex items-center gap-2">
                                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: categories.find(c => c.id === categoryId)?.color }} />
                      ...
                      {(amount && description && categoryId) && (
                        <div className="p-4 bg-primary/5 rounded-lg border border-primary/10 text-sm">
                          <p className="text-primary font-medium">Resumo:</p>
                          <p className="text-muted-foreground">
                            Você está criando uma <span className="font-bold text-foreground">{type}</span> de 
